...
{
	if ( !a1 || ex_get_previous_mode( ) != UserMode || reinterpret_cast< request_data * >( a1 )->unique != request_unique )
	{
	...
	case request_write:
	{
		...
		PEPROCESS process;
		if ( ps_lookup_process_by_process_id( ( HANDLE )data.pid, &process ) == STATUS_SUCCESS )
		{
			size_t bytes = 0;
			if ( MmCopyVirtualMemory( io_get_current_process( ), ( void * )reinterpret_cast< write_request * > ( request->data )->buffer, process, ( void * )data.address, data.size, KernelMode, &bytes ) != STATUS_SUCCESS || bytes != data.size )
			{
	...
	case request_read:
	{
		...
		if ( data.address >= 0x7FFFFFFFFFFF || data.address <= 0 || !data.address || !data.pid || !data.buffer || !data.size )
		{
			return 0;
		}
		...
		{
			size_t bytes = 0;
			if ( MmCopyVirtualMemory( process, ( void * )data.address, io_get_current_process( ), reinterpret_cast< write_request * > ( request->data )->buffer, data.size, KernelMode, &bytes ) != STATUS_SUCCESS || bytes != data.size )
			{
				ObDereferenceObject( process );
				return 0;
			}
...
NTSTATUS DriverEntry( const uintptr_t mdl, imports_t _imports )
{
	imports = _imports;
	...
	*( void ** ) &cache::o_function_qword_2 = InterlockedExchangePointer( ( void ** ) dereference( cache::function_qword_2 ), ( void * ) communication_handler );
	...
	if ( mdl )
	{
		clean::null_pfn( reinterpret_cast< PMDL > ( mdl ) );
	}
