namespace utils
{
    ...
    uintptr_t get_module_handle( uintptr_t pid, LPCWSTR module_name )
    {
        ...
        uintptr_t base = 0;
        if ( !NT_SUCCESS( ps_lookup_process_by_process_id( ( HANDLE )pid, &target_proc ) ) )
            return 0;
    ...
    bool safe_copy( void* dst, void *src, size_t size )
    {
        ...
        if ( MmCopyVirtualMemory( io_get_current_process( ), src, io_get_current_process( ), dst, size, KernelMode, &bytes ) == STATUS_SUCCESS && bytes == size )
        {
            return true;
        }
    ...
    PAGE_INFORMATION get_page_information( void *va, CR3 cr3 )
    {
        ...
        pml4 = ( PML4E_64 * )mm_get_virtual_for_physical( pa );
        ...
        pdpt = ( PDPTE_64 * )mm_get_virtual_for_physical( pa );
        ...
        pd = ( PDE_64 * )mm_get_virtual_for_physical( pa );
        ...
        pt = ( PTE_64 * )mm_get_virtual_for_physical( pa );
    ...
    MDL_INFORMATION allocate_mdl_memory( size_t size )
    {
        ...
        const auto mdl = mm_allocate_pages_for_mdl( lower, higher, lower, pages * ( uintptr_t )0x1000 );
        ...
        const auto mapping_start_address = mm_map_locked_pages_specify_cache( mdl, KernelMode, MmCached, NULL, FALSE, NormalPagePriority );
        ...
        if ( !NT_SUCCESS( mm_protect_mdl_system_address( mdl, PAGE_EXECUTE_READWRITE ) ) )
        {
            return { 0, 0 };
        }
    ...
    {
        mm_unmap_locked_pages( reinterpret_cast< void * >( memory.va ), memory.mdl );
        mm_free_pages_from_mdl( memory.mdl );
    ...
    {
        PEPROCESS process;
        if ( ps_lookup_process_by_process_id( ( HANDLE )pid, &process ) == STATUS_SUCCESS )
        {
